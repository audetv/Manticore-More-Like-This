  ...
  <div class="paginationControl">
    <!-- First page link -->
      <?php if (isset($previous)): ?>
        <a href="<?php echo $url; ?>?<?php $_GET['start'] = $first * $offset;
    ...
    <!-- Previous page link -->
      <?php if (isset($previous)): ?>
        <a href="<?php echo $url; ?>?<?php $_GET['start'] = ($previous - 1) * $offset;
        echo http_build_query($_GET); ?>">
    ...
    <!-- Numbered page links -->
      <?php for ($page = ($current - $range); $page < ($current + $range + 1); $page++): ?>
          <?php if ($page > 0 && $page <= $pageCount): ?>
              <?php if ($page != $current): ?>
            <a href="<?php echo $url; ?>?<?php $_GET['start'] = ($page - 1) * $offset;
            echo http_build_query($_GET); ?>">
                <?php echo $page; ?>
            </a> |
              <?php else: ?>
    ...
    <!-- Next page link -->
      <?php if (isset($next)): ?>
        <a href="?<?php echo $url; ?><?php $_GET['start'] = ($next - 1) * $offset;
        echo http_build_query($_GET); ?>">
    ...
    <!-- Last page link -->
      <?php if (isset($next)): ?>
        <a href="<?php echo $url; ?>?<?php $_GET['start'] = ($last - 1) * $offset;
        echo http_build_query($_GET); ?>">
