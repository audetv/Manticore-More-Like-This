...
if (isset($_POST['send'])) {
    ...
    if ($boolean == true) {
        $content_query = '@text ( ' . implode('| ', array_column($content_keywords, 'query')) . ')';
        $title_query = '@book_name ( ' . implode('| ', array_column($title_keywords, 'query')) . ')';
    } else {
        $content_query = '@text "' . implode(' ', array_column($content_keywords, 'query')) . '"/' . "$minimum_should_match";
        $title_query = '@book_name "' . implode(' ', array_column($title_keywords, 'query')) . '"/' . "$minimum_should_match";
    }
    if ($ranker == 'expr') {
    ...
    $stmt = $conn->prepare("SELECT *,WEIGHT() as w FROM $index WHERE MATCH(:mlk) LIMIT $start,$offset OPTION ranker=$ranking, idf='plain,tfidf_unnormalized',field_weights=(book_name=1,text=1)");
    ...
    $sphinxql = interpolateQuery("SELECT *,WEIGHT() as w FROM $index WHERE MATCH(:mlk) LIMIT $start,$offset OPTION ranker=$ranking, idf='plain,tfidf_unnormalized',field_weights=(book_name=1,text=1", array('mlk' => $mlk_query));
} else {
