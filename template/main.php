        ...
        <div class="card">
          ...
          <div class="collapse" id="collapseExample">
            <div class="card-body">
              ...
              <div class="form-group">
                <div class="form-check">
                  <input type="checkbox" name="tfidf_boost"
                         value="1" <?= isset($_POST['tfidf_boost']) ? 'checked="checked"' : (!isset($_POST['send']) ? 'checked="checked"' : '') ?>><label>Enable
                    boost on words based on tdidf</label>
                </div>
              </div>
              <div class="form-group">
                <div class="form-check">
                  <input type="checkbox" name="boolean"
                         value="1" <?= isset($_POST['boolean']) ? 'checked="checked"' : '' ?>><label>Perform boolean
                    instead of quorum</label>
                </div>
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <select name="ranker" class="form-control">
                    <option value="proximity_bm25"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'proximity_bm25' ? 'selected' : '' ?>>
                      proximity_bm25
                    </option>
                    <option value="bm25"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'bm25' ? 'selected' : '' ?>>
                      bm25
                    </option>
                    <option value="none"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'none' ? 'selected' : '' ?>>
                      none
                    </option>
                    <option value="wordcount"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'wordcount' ? 'selected' : '' ?>>
                      wordcount
                    </option>
                    <option value="matchany"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'matchany' ? 'selected' : '' ?>>
                      matchany
                    </option>
                    <option value="sph04"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'sph04' ? 'selected' : '' ?>>
                      sph04
                    </option>
                    <option value="expr"<?= isset($_POST['ranker']) && $_POST['ranker'] == 'expr' ? 'selected' : (!isset($_POST['send']) ? 'selected' : '') ?>>
                      expr
                    </option>
                  </select>
                  <input type="text" name="rankerexpr"
                         value="<?= isset($_POST['rankerexpr']) ? $_POST['rankerexpr'] : (!isset($_POST['send']) ? 'sum(atc*1000)' : '') ?>"
                         class="form-control <?= isset($_POST['ranker']) && $_POST['ranker'] == 'expr' ? 'd-block' : (!isset($_POST['send']) ? 'd-block' : 'd-none') ?>">
        ...
        <div class="row">
          ...
          <div class="col-sm-12">
            <h3>
              ...
              <?php else: ?>
                <span><?= highlightidf($doc['book_name'], $title_keywords, $title_keywords_max_tf_idf) ?></span> </h3>
              <?php endif; ?>
            ...
            <div style="height:200px;overflow:scroll" class="doccontent">
                <?php if (($field != 'book_name') && isset($doc['w'])): ?>
                    <?php if ($doc['w'] < $thresholdscore): ?>
                    <b style="color:grey;opacity:0.2"><?= $doc['text']; ?></b>
                    <?php else: ?>
                        <?= highlightidf($doc['text'], $content_keywords, $content_keywords_max_tf_idf) ?>
                    <?php endif; ?>
